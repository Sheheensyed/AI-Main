...
import { addCase, addNewStep, addStepToOperation, deleteOperation, deleteSingleSteps, deleteStepFromOperation, editSingleSteps, getAllSteps, getDutsById, getOperationsByCaseId, getSingleCase, mapSteps, saveTemplateToDB, updateStep } from '../services/allApi';
...
function Search() {
    ...
    const handleDeleteStepFromOperation = async (operationId, stepId) => {
        try {
            const response = await deleteStepFromOperation(operationId, stepId);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                setUserQuery(caseData.user_query);

                // Flatten all steps from each operation
                const steps = (caseData?.operations ?? []).flatMap((op) => op.steps || []);

                const operations = caseData?.operations ?? [];

                console.log("Steps:", steps);
                console.log("Operations:", operations);

                if (!Array.isArray(steps) || steps.length === 0) {
                    console.warn("No steps generated by Gemini.");
                    setWarningMessage("No steps were generated by Gemini. Please edit manually or try again.");
                    return;
                }

                const parsedSteps = steps.map((s) => ({
                    ...s,
                    content: { step: s.content },
                }));
    ...
    const handleShowInput = async (opId) => {
        try {
            ...
            if (res?.data?.data) {
                ...
                setCurrentStepId(newStep.id); // keep focus
    ...
    const handleExecute = async () => {
        try {
            ...
            const cleanedSteps = steps
                ...
                .map(step => `${step.device} :: ${step.step.trim()}`);

            const response = await mapSteps(id, cleanedSteps);
            if (response.data && response.data.caseId) {
                const updatedCase = await getSingleCase(response.data.caseId); // 👈 fetch updated case
                setCaseId(updatedCase.id);
                setSteps(response.data.steps);
                console.log("Steps from backend:", Array.isArray(response.data.steps), response.data.steps);
                setMappedSteps(response.data.mapped_steps);
                navigate(`/lists/${response.data.caseId}`); // ✅ Go to /list after successful mapping
            }
        } catch (error) {
            console.log(error);
        } finally {
                    ...
                    <>
                        ...
                        <div
                            ...
                            >
                                ...
                                {warningMessage && (
                                    <div className="alert alert-warning mt-3">{warningMessage}</div>
                                )}
                                            ...
                                            <div className="ps-3">
                                                ...
                                                <Form.Group className="mb-3">
                                                            ...
                                                            <div key={step.id} className="d-flex align-items-center mb-2">
                                                                {step.id === currentStepId ? (
                                                                    <Form.Control
                                                                        type="text"
                                                                        autoFocus
                                                                        value={stepContents[step.id] !== undefined
                                                                        ...
                                                                        onBlur={() => {
                                                                            if ((stepContents[step.id] || "").trim() !== "") {
                                                                                setCurrentStepId(null); // only exit edit mode if some content was typed
                                                                            }
                                                                        }}
                                                                    ...
                                                                    <div className='position-relative w-100 me-2'>
                                                                        ...
                                                                        />
                                                                        <Badge
                                                                            className="position-absolute bg-white top-50 translate-middle-y end-0 me-2 opacity-75"
                                                                            ...
                                                                            onClick={() => {
                                                                                ...
                                                                                setStepContents(prev => ({
                                                                                    ...prev,
                                                                                    [step.id]: prev[step.id] !== undefined
                                                                                        ? prev[step.id]
                                                                                        : (
                                                                                            typeof step.content === "object"
                                                                                                ? step.content.step || step.content.description || JSON.stringify(step.content)
                                                                        ...
                                                                        >
                                                                            {/* ✏️ */}
                                                                            <FontAwesomeIcon icon={faPenToSquare} className='text-dark opacity-50' />
                                                                        </Badge>
                                                                    </div>
                                                                )}
                                                                ...
                                                                {/* ❌ Delete Step */}
                                                                <Badge
                                                                    className="bg-white text-danger fs-6"
                                                                    style={{ cursor: "pointer", minWidth: "24px", textAlign: "center" }}
                                                                    onClick={() => handleDeleteStepFromOperation(operation.id, step.id)}
                                                                    title="Delete step"
                                                                >
                                                                    ×
                                                                </Badge>
                                                    ...
                                                    <button
                                                        ...
                                                        style={{ height: "30px", width: "90px", fontSize: "12px" }}
